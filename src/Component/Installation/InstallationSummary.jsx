...
function InstallationSummary() {
  ...
  const parseStreamingResponse = (chunk) => {
    try {
      // Multiple JSONs can be streamed together — split and parse individually
      ...
      const newStatuses = messages.map((msg) => {
      ...
      setProgressStatus((prev) => [...prev, ...newStatuses]);
    } catch (error) {
  ...
  const getStatusIcon = (type, status) => {
    if (type === "equipment-complete" || status === "success") {
