...
function PmsPage() {
  ...
  }, [allPms, decodedCustomerCode, decodedRegion, decodedCity]);
  ...
  const fetchPmsWithSearch = async (query = "", page = 1) => {
    ...
    try {
      const url = query
        ? `${process.env.REACT_APP_BASE_URL}/upload/pmsearch?q=${encodeURIComponent(query)}&page=${page}&limit=10`
        : null;
  ...
  }, [searchQuery, allPms, decodedCustomerCode, decodedRegion, decodedCity]);

  const handleBackToCustomers = () => {
    navigate(`/pm-customers/${encodeURIComponent(decodedRegion)}/${encodeURIComponent(decodedCity)}`);
  };
  ...
  const getStatusIcon = (status) => {
    const { AlertTriangle, XCircle, CheckCircle, Calendar } = require("lucide-react");
    switch (status) {
      case "Due":
