...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/collections/login`, {
      ...
      if (errorCode === "DEVICE_MISMATCH") {
        ...
        toast.error(errorMessage || "Access denied to mobile app. Contact admin.")
      } else {
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
          <div className="flex flex-col items-center text-white">
            <div className="bg-white rounded-full p-2 mb-2 shadow-lg">
              <img src="/Skanray-logo.png" alt="Skanray Logo" className="w-8 h-8 object-contain" />
            </div>
            <h2 className="text-sm font-bold tracking-wide">SKANRAY SERVICE PORTAL</h2>
          </div>
        </div>
        ...
        <div className="px-6 py-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <label htmlFor="employeeId" className="text-xs font-medium text-gray-700 block">
                Employee ID
              </label>
              ...
              {formErrors.employeeId && <p className="text-red-500 text-xs">{formErrors.employeeId}</p>}
            </div>
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 flex flex-col items-center space-y-3 shadow-2xl mx-4">
            ...
            <p className="text-gray-700 font-medium text-sm">Authenticating...</p>
            <p className="text-gray-500 text-xs text-center">Please wait while we verify your credentials</p>
          </div>
        </div>
      )}
    </div>
